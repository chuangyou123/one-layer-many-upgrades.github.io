...
function formatBAN(decimal, precision) {
   ...
   // 3b: 如果塔高本身很大（例如 F1e100），我们需要使用 BAN 格式化高度。
   else {
...
function infinityFormat(decimal, precision=3) {
   ...
   if (decimal.lt(pow1024.tetrate(4))) {
       return format(decimal.log(pow1024).floor()) + "∞" // 添加因为开头显示 1* 没有意义
   }
   return "∞^(" + decimal.slog(pow1024) + ")" // 非常特殊
}
...
function formatTime(s) {
   if (s < 60) return (Math.floor(s)) + "秒 " + ((s % 1) * 1000).toPrecision(3) + "毫秒"
